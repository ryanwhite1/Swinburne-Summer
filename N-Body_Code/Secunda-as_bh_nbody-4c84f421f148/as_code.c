...
#define BETA            1;		//what fraction of a hill radius a merger takes place at
#define NPLMAX          10
...
typedef double vektor[glnmax];
 ...
 int CLOSE,COLLISION,BODY_CREATE, PRINT_TIME, ESCAPE, GASDRAG, MIGRATION_TYPE1, ADIABATIC, NPL, NPHI, LamIndex[MODES], UPDATE_OPACITY;
...
double *Body, *radii, *sigma, *alpha, *temp, *beta,*Y, *F_stoc, *Force;
{
  ...
  double Torque[2], Damp[3], xrel[3], vrel[3]; 
    ...
    /*saves a position and velocity vector for the two nearest bodies*/
      ...
      if (dmin<rhill(Body[imin],Body[jmin],Body0)){
	CLOSE=1;
      }
      else{
	CLOSE=0;
...
{  
  ...
  /* file declarations                                                                                     */
  /*********************************************************************************************************/
  fin1 = fopen("test/ex_elements.dat","r"); //this file contains the starting positions and mass of the BHs
  ...
  ptr_myfile=fopen("test/bout.bin","wb"); //this is the main output file
  fin_time = fopen(timefile,"r"); //this file has the temperature, surface density and their gradients for all disk radii
  fout3= fopen("test/analysis.txt","w"); //this file has info on all of the mergers
  /*********************************************************************************************************/
  ...
  max_time =  1.5e5*365.25; /* lenght of the numerical integration in days                 */
